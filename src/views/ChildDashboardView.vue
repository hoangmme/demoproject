...
const matchSingleCondition = (item, cond) => {
  ...
  if (op.startsWith('count_') || field === 'dieu_kien_dem' || field === '_tripCount') {
    ...
    if (isNaN(count)) {
      const personTrips = Array.isArray(item.trips) ? item.trips : (Array.isArray(item.rawPerson?.trips) ? item.rawPerson.trips : []);
  ...
  const isCurrentRelativesTable = currentDashboardConfig.value?.source === 'relatives' || item.isRelative;
  const isCurrentPersonnelTable = currentDashboardConfig.value?.source === 'personnel';
