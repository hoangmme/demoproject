...
const dialogTableIdForRecord = computed(() => {
  ...
  // Luôn ưu tiên Cán bộ nếu bản ghi đã quy về Bảng Đứng Đầu
  if (rec._recordType === 'personnel' || rec.isPersonnel || (rec.departmentName && !rec.departureDate && !rec.relationshipName)) {
    return 'personnel';
  }
...
function openPersonnelDetail(record) {
  if (!record) return;
  const curSource = currentDashboardConfig.value?.source || topicId.value || 'trips';

  let masterPerson = record;
  let targetSection = 'info';
  let targetRecordId = record.id || record.uniqueKey || null;

  if (curSource !== 'personnel') {
    const linked = getLinkedRowsByConfig(record, curSource, 'personnel', personnelStore);
    if (linked && linked.length > 0) {
      masterPerson = { ...linked[0], _recordType: 'personnel' };
      targetSection = curSource;
    } else if (personnelStore.findParentPersonForTrip && (curSource === 'trips' || record.departureDate || record.ngay_xuat_canh)) {
      const p = personnelStore.findParentPersonForTrip(record);
      if (p) {
        masterPerson = { ...p, _recordType: 'personnel' };
        targetSection = 'trips';
      }
    } else if (personnelStore.findParentPersonForRelative && (curSource === 'relatives' || record.relationshipName || record.relativeName)) {
      const p = personnelStore.findParentPersonForRelative(record);
      if (p) {
        masterPerson = { ...p, _recordType: 'personnel' };
    ...
    masterPerson = { ...record, _recordType: 'personnel' };
  }

  activePersonData.value = masterPerson;
